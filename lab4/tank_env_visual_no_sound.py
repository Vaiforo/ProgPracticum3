# -*- coding: utf-8 -*-
"""
laba4_improved_no_sound.py

Улучшенная версия среды с танком на PyGame без звуков:
- Поворот танка с изображением
- Отображение угла и позиции
...
def draw_tank_with_rotation(tank, surface):
    # Создаём поверхностный объект для танка (треугольник)
    tank_img = pygame.Surface((GRID_SIZE, GRID_SIZE), pygame.SRCALPHA)
    pygame.draw.polygon(tank_img, (0, 0, 255), [(0, GRID_SIZE), (GRID_SIZE, GRID_SIZE), (GRID_SIZE / 2, 0)])
    rotated_img = pygame.transform.rotate(tank_img, -tank.angle)
    rect = rotated_img.get_rect(center=(tank.x * GRID_SIZE + GRID_SIZE / 2, tank.y * GRID_SIZE + GRID_SIZE / 2))
