# -*- coding: utf-8 -*-
"""
train_agent_refactored.py

Обучение агента с использованием Q-learning
в кастомной среде танка (TankEnv).
"""
...
from tank_env4 import TankEnv  # кастомная среда
...
def discretize_observation(observation: np.ndarray) -> tuple:
    """
    Переводит наблюдение среды в дискретное состояние.
    Округляет координаты и расстояния, угол к ближайшему кратному 15°.
    """
...
def select_action(state: tuple, q_values: dict, epsilon: float) -> int:
    """
    Выбирает действие по ε-жадной политике:
    случайное действие с вероятностью epsilon,
    иначе действие с максимальным Q-значением.
    """
    if random.random() < epsilon:
        return random.randint(0, 4)  # 5 действий в среде TankEnv
    return int(np.argmax(q_values[state]))

def run_training():
    # Создаем окружение с заданными параметрами
    wall_cells = {(3,5), (4,5), (5,5), (10,2), (10,3), (10,4), (15,8), (16,8), (17,8)}
    goal_position = (18,10)
    environment = TankEnv(width=20, height=15, wall_coords=wall_cells, goal=goal_position)

    # Инициализируем Q-таблицу с дефолтными значениями 0 для каждого действия
    Q_table = defaultdict(lambda: np.zeros(environment.action_space.n, dtype=np.float32))
    ...
    for episode in range(1, TOTAL_EPISODES + 1):
        ...
        for step in range(MAX_STEPS_PER_EPISODE):
            ...
            # Q-learning обновление по формуле
        ...
        # Логирование награды за эпизод
        rewards_log.append(cumulative_reward)

        # Уменьшение epsilon, но не ниже минимального
        epsilon = max(EPSILON_MIN, epsilon * EPSILON_DECAY_FACTOR)

        # Вывод прогресса каждые 50 эпизодов
        if episode % 50 == 0:
            print(f"Эпизод {episode:3d} | Награда: {cumulative_reward:6.1f} | ε={epsilon:.3f}")
